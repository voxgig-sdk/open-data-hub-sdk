...
import (
	...
	omni "github.com/voxgig-sdk/open-data-hub-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.OpenDataHubError); is {
				out := map[string]any{
					"name":    "OpenDataHubError",
...
func omniCtx(arg any, client *sdk.OpenDataHubSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.OpenDataHubSDK); is {
			client = live
