...
class FtCtrl extends OpenDataHubControl
{
...
class FtClient
{
    ...
    private OpenDataHubUtility $utility;

    public function __construct(OpenDataHubUtility $utility, array $options)
    {
    ...
    public function get_utility(): OpenDataHubUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (OpenDataHubContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public OpenDataHubContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (OpenDataHubContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = OpenDataHubConfig::make_feature($name);
    ...
    private static function buildUrl(OpenDataHubSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof OpenDataHubError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new OpenDataHubResponse($fetched);
        }
    ...
    private function fail(OpenDataHubContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (OpenDataHubContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
