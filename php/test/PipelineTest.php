...
require_once __DIR__ . '/../opendatahub_sdk.php';
...
{
    private static function utility(): OpenDataHubUtility
    {
        return new OpenDataHubUtility();
    }
    ...
    private static function ctx(array $over = []): OpenDataHubContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new OpenDataHubContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new OpenDataHubOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof OpenDataHubError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new OpenDataHubOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = OpenDataHubMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new OpenDataHubOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = OpenDataHubMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new OpenDataHubOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = OpenDataHubMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new OpenDataHubSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = OpenDataHubMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new OpenDataHubResponse([]),
            'result' => new OpenDataHubResult([]),
        ]);
        [, $err] = OpenDataHubMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new OpenDataHubSpec([]),
            'response' => null,
            'result' => new OpenDataHubResult([]),
        ]);
        [, $err] = OpenDataHubMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new OpenDataHubSpec(['step' => 's']),
            'response' => new OpenDataHubResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new OpenDataHubResult([]),
        ]);
        [, $err] = OpenDataHubMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new OpenDataHubSpec(['step' => 's']),
            'response' => new OpenDataHubResponse(self::resp(200, ['v' => 1])),
            'result' => new OpenDataHubResult([]),
        ]);
        [, $err] = OpenDataHubMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new OpenDataHubSpec(['step' => 's']),
            'response' => new OpenDataHubResponse(self::resp(200, ['v' => 2])),
            'result' => new OpenDataHubResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        OpenDataHubMakeResponse::call($ctx);
    ...
    {
        $preset = new OpenDataHubResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new OpenDataHubSpec([]),
            'response' => new OpenDataHubResponse([]),
            'result' => new OpenDataHubResult([]),
        ...
        [$got, $err] = OpenDataHubMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new OpenDataHubResult([])]);
        [, $err] = OpenDataHubMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new OpenDataHubSpec([]), 'result' => null]);
        [, $err] = OpenDataHubMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new OpenDataHubSpec(['step' => 's']),
            'result' => new OpenDataHubResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new OpenDataHubOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = OpenDataHubMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new OpenDataHubSpec(['step' => 's']),
            'result' => new OpenDataHubResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new OpenDataHubOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = OpenDataHubMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new OpenDataHubResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new OpenDataHubSpec([]),
            'result' => new OpenDataHubResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = OpenDataHubMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = OpenDataHubMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new OpenDataHubError('boom', 'boom');
        $utility->fetcher = function (OpenDataHubContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new OpenDataHubSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = OpenDataHubMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (OpenDataHubContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = OpenDataHubMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(OpenDataHubResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        OpenDataHubMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (OpenDataHubContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new OpenDataHubSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = OpenDataHubMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new OpenDataHubResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new OpenDataHubSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = OpenDataHubMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = OpenDataHubMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = OpenDataHubMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new OpenDataHubResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, OpenDataHubDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new OpenDataHubResult(['ok' => false])]);
        $this->expectException(OpenDataHubError::class);
        OpenDataHubDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new OpenDataHubResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', OpenDataHubMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        OpenDataHubMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new OpenDataHubBaseFeature();
        $b = new OpenDataHubBaseFeature();
        OpenDataHubFeatureAdd::call($ctx, $a);
        OpenDataHubFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): OpenDataHubBaseFeature
    {
        $f = new OpenDataHubBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        OpenDataHubFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): OpenDataHubContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new OpenDataHubSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = OpenDataHubPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        OpenDataHubPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new OpenDataHubResponse(['headers' => null]),
            'result' => new OpenDataHubResult([]),
        ]);
        OpenDataHubResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        OpenDataHubResultBody::call($ctx);
