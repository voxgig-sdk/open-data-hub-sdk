...
class OpenDataHubMakeError
{
    public static function call(?OpenDataHubContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new OpenDataHubContext([], null);
        }
        $op = $ctx->op ?? new OpenDataHubOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new OpenDataHubResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof OpenDataHubError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new OpenDataHubError('', $msg, $ctx);
        ...
        if ($err instanceof OpenDataHubError) {
