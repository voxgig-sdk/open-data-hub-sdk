...
// OpenDataHub SDK GetDataBrowser entity
...
class GetDataBrowserEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new GetDataBrowserEntity($this->_client, $opts);
    }

    /**
     * @param GetDataBrowser|array $args GetDataBrowser data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = OpenDataHubHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return GetDataBrowser|array The current GetDataBrowser data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of GetDataBrowser fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = OpenDataHubHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of GetDataBrowser fields).
     */
    ...
    /**
     * List GetDataBrowser items matching the given filter.
     *
     * @param GetDataBrowserListMatch|array|null $reqmatch Match filter (any subset
     *   of GetDataBrowser fields) as an assoc-array; GetDataBrowserListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return GetDataBrowser[]|array A list of GetDataBrowser items as assoc-arrays at
     *   the SDK boundary; throws OpenDataHubError on failure (item-5 convention).
     */
