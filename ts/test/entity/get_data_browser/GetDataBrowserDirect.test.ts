...
describe('GetDataBrowserDirect', async () => {
  ...
  test('direct-list-get_data_browser', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-get_data_browser', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['OPEN_DATA_HUB_TEST_GET_DATA_BROWSER_ENTID']
