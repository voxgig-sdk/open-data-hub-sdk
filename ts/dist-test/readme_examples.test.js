...
const TEST_SEED = { "entity": { "get_data_browser": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+OpenDataHubSDK\b/.test(code) ||
        /\bOpenDataHubSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+OpenDataHubSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/OpenDataHubSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
