...
boru-driven command-line client **and** interactive REPL for the OpenDataHub
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/open-data-hub-cli)
make build

# 2. See usage (words, entities, env vars)
./open-data-hub-cli --help

# 3. Provide credentials once, via the environment
export OPEN_DATA_HUB_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./open-data-hub-cli list get_data_browser

# 5. Override the API base URL for a single call
OPEN_DATA_HUB_BASE=https://api.example.com ./open-data-hub-cli list get_data_browser

# 6. No arguments -> interactive REPL
./open-data-hub-cli
open-data-hub> list get_data_browser
open-data-hub> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/open-data-hub-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/open-data-hub-cli list get_data_browser
   ```
...
Run with no arguments to open a REPL (prompt `open-data-hub>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `get_data_browser`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/open-data-hub-cli`. |
...
(`./open-data-hub-cli <expr>`) and interactively (the REPL), and expressions compose the
