...
[MCP](https://modelcontextprotocol.io) server exposing the OpenDataHub SDK as
two agent tools — `open-data-hub_list` and `open-data-hub_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/open-data-hub-mcp)
make build

# 2. Provide credentials via the environment
export OPEN_DATA_HUB_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user open-data-hub \
  -- /absolute/path/to/open-data-hub-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./open-data-hub-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// open-data-hub_list: first page of records
{ "entity": "get_data_browser" }
{ "entity": "get_data_browser", "query": { } }

// open-data-hub_load: one record by id
{ "entity": "get_data_browser", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/open-data-hub-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user open-data-hub \
     -- "$PWD"/dist/*/open-data-hub-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `open-data-hub_list` and `open-data-hub_load` tools now appear
   in new sessions. Ask the agent to *"list get_data_browser using open-data-hub"*
   and it calls `open-data-hub_list` with `{"entity":"get_data_browser"}`.
...
```sh
export OPEN_DATA_HUB_APIKEY=sk_live_xxx            # API key
export OPEN_DATA_HUB_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `open-data-hub_list` tool
...
### Call the `open-data-hub_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `open-data-hub_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `open-data-hub_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `OPEN_DATA_HUB_APIKEY` | API key sent with every request. |
| `OPEN_DATA_HUB_BASE` | Optional override of the API base URL. |
...
```sh
./open-data-hub-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"open-data-hub_load","arguments":{"entity":"get_data_browser","query":{"id":1}}}}'
```
