...
from opendatahub_sdk.utility import register

# Load features
from opendatahub_sdk.feature.base_feature import OpenDataHubBaseFeature
from opendatahub_sdk.features import _has_feature, _make_feature
...
class OpenDataHubSDK:
    ...
    def get_utility(self):
        return OpenDataHubUtility.copy(self._utility)
    ...
    def graphql(self, query, variables=None, ctrl=None):
        ...
        if isinstance(errors, list) and 0 < len(errors):
            ...
            res["ok"] = False
            res["err"] = Exception("OpenDataHubSDK: graphql: " + str(msg))
            res["graphql"] = errors

        return res


    def GetDataBrowser(self, data=None) -> "GetDataBrowserEntity":
        """Entity factory: client.GetDataBrowser().list() / client.GetDataBrowser().load({"id": ...})."""
        from opendatahub_sdk.entity.get_data_browser_entity import GetDataBrowserEntity
        return GetDataBrowserEntity(self, data)



    @classmethod
    def test(cls, testopts=None, sdkopts=None) -> "OpenDataHubSDK":
        if sdkopts is None:
            sdkopts = {}
