...
require_relative 'OpenDataHub_types'
...
class OpenDataHubSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = OpenDataHubHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = OpenDataHubHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, OpenDataHubFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    OpenDataHubUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = OpenDataHubHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = OpenDataHubHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = OpenDataHubHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = OpenDataHubHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = OpenDataHubError.new(
        "graphql_error", "OpenDataHubSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.GetDataBrowser.list / client.GetDataBrowser.load({ "id" => ... })
  def GetDataBrowser(data = nil)
    require_relative 'entity/get_data_browser_entity'
    GetDataBrowserEntity.new(self, data)
  end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = OpenDataHubSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
