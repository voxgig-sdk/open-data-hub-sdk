...
class GetDataBrowserEntity
  ...
  def make
    opts = @_entopts.dup
    GetDataBrowserEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = OpenDataHubHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [GetDataBrowser, Hash] the current GetDataBrowser data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = OpenDataHubHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of GetDataBrowser fields)
  def match_get
  ...
  #
  # @param reqmatch [GetDataBrowserListMatch, Hash, nil] match filter (any subset of
  #   GetDataBrowser fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<GetDataBrowser>, Array] the matching GetDataBrowser items; raises OpenDataHubError on failure
