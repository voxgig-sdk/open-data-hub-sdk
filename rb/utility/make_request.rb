...
module OpenDataHubUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = OpenDataHubResponse.new({})
    ...
    if fetch_err
      ...
      response = OpenDataHubResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = OpenDataHubResponse.new(fetched)
    else
