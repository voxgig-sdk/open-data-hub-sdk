...
Open Data Hub API client, generated from the OpenAPI spec.

The Open Data Hub API provides programmatic access to various datasets and resources.

Learn more about Open Data Hub API at [opendatahub.readthedocs.io/en/latest](https://opendatahub.readthedocs.io/en/latest/).

This is an unofficial SDK for the Open Data Hub public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
This SDK exposes the API as a small set of **semantic entities** — GetDataBrowser — that you
...
const items = await client.GetDataBrowser().list()
```
...
```go
client := sdk.Test()
result, err := client.GetDataBrowser(nil).List(
    nil, nil,
)
```
...
```bash
cd go-mcp && go build -o open-data-hub-mcp .
```
...
```json
{
  "mcpServers": {
    "open-data-hub": {
      "command": "/abs/path/to/open-data-hub-mcp"
    }
  }
}
```
...
| --- | --- | --- |
| **GetDataBrowser** | The GetDataBrowser entity (list). | `/` |
...
# List all getdatabrowsers (returns a list, raises on error)
getdatabrowsers = client.GetDataBrowser().list()
...
// List all getdatabrowsers (returns an array; throws on error)
$getdatabrowsers = $client->GetDataBrowser()->list();
print_r(array_map(fn($item) => $item->data_get(), $getdatabrowsers));
```

### Golang

```go
import sdk "github.com/voxgig-sdk/open-data-hub-sdk/go"

client := sdk.New()

// List all getdatabrowsers
getDataBrowsers, err := client.GetDataBrowser(nil).List(nil, nil)
if err != nil {
    panic(err)
}
fmt.Println(getDataBrowsers)
```
...
# List all getdatabrowsers (returns an Array; raises on error)
getdatabrowsers = client.GetDataBrowser.list
puts getdatabrowsers
```
...
Generated from the Open Data Hub API OpenAPI spec by [@voxgig/sdkgen](https://github.com/voxgig/sdkgen).
